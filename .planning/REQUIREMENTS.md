...
- [ ] **SKILL-01**: Root `SKILL.md` (~200 lines) includes: auth setup section documenting `twentythree auth credentials` as a prerequisite, command syntax overview, full resource index linking to all 22 reference files, `--agent` flag documentation, and `allowed-tools: Bash(twentythree *)` declaration
