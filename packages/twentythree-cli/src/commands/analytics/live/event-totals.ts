...
/**
 * Analytics live event-totals command — returns live/webinar event totals analytics.
 *
 * Uses GET /analytics/data/live/event/totals (slash, NOT hyphen in API path).
 * No p/size — totals does not support pagination.
 */
export default class AnalyticsLiveEventTotals extends AuthenticatedCommand<typeof AnalyticsLiveEventTotals> {
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsLiveEventTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/live/event/totals', {
      params: {
        query: {
