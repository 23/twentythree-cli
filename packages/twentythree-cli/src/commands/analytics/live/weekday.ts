...
/**
 * Analytics live weekday command — returns live/webinar analytics by day of week.
 *
 * Uses GET /analytics/data/live/weekday (supports p/size pagination).
 */
export default class AnalyticsLiveWeekday extends AuthenticatedCommand<typeof AnalyticsLiveWeekday> {
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsLiveWeekday)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/live/weekday', {
      params: {
        query: {
