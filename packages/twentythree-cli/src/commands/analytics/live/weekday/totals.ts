...
/**
 * Analytics live weekday totals command — returns aggregated live/webinar analytics by weekday totals.
 *
 * Uses GET /analytics/data/live/weekday/totals (no p/size — totals does not support pagination).
 */
export default class AnalyticsLiveWeekdayTotals extends AuthenticatedCommand<typeof AnalyticsLiveWeekdayTotals> {
  static description = 'Get aggregated live/webinar analytics by weekday - totals'
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsLiveWeekdayTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/live/weekday/totals', {
      params: {
        query: {
    ...
    const headers = ['Day', 'Plays', 'Peak Viewers', 'Engagement', 'Playrate']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
