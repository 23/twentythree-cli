 ...
 * Satisfies ANL-06.
 */
export default class AnalyticsConversionsTotals extends AuthenticatedCommand<typeof AnalyticsConversionsTotals> {
  ...
  public async run(): Promise<void | object> {
    ...
    const { data, error } = await this.apiClient.GET('/analytics/data/conversions/totals', {
      params: {
        query: {
    ...
    const headers = ['Conversions', 'Views', 'Visits', 'Engagement']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
