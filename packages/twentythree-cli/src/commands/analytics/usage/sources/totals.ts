...
/**
 * Analytics usage sources totals command — returns aggregated usage analytics by source.
 *
 * Uses GET /analytics/data/usage/sources/totals.
 * No pagination — the totals endpoint does not support p/size.
 */
export default class AnalyticsUsageSourcesTotals extends AuthenticatedCommand<typeof AnalyticsUsageSourcesTotals> {
  static description = 'Get aggregated usage analytics by source - totals'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Source', 'Plays', 'Engagement', 'Traffic'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageSourcesTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/sources/totals', {
      params: {
        query: {
    ...
    const headers = ['Source', 'Plays', 'Engagement', 'Traffic']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
