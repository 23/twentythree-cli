...
/**
 * Analytics usage locations totals command — returns aggregated usage analytics by location.
 *
 * Uses GET /analytics/data/usage/locations/totals.
 * No pagination — the totals endpoint does not support p/size.
 */
export default class AnalyticsUsageLocationsTotals extends AuthenticatedCommand<typeof AnalyticsUsageLocationsTotals> {
  static description = 'Get aggregated usage analytics by location - totals'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Location', 'Plays', 'Engagement', 'Traffic'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageLocationsTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/locations/totals', {
      params: {
        query: {
    ...
    const headers = ['Location', 'Plays', 'Engagement', 'Traffic']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
