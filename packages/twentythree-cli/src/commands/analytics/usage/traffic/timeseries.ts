...
/**
 * Analytics usage traffic timeseries command — returns traffic usage analytics time series data.
 *
 * Uses GET /analytics/data/usage/traffic/timeseries.
 * No pagination — the timeseries endpoint does not support p/size.
 */
export default class AnalyticsUsageTrafficTimeseries extends AuthenticatedCommand<typeof AnalyticsUsageTrafficTimeseries> {
  static description = 'Get traffic usage analytics - time series'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Date', 'Traffic Type', 'Plays', 'Engagement'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageTrafficTimeseries)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/traffic/timeseries', {
      params: {
        query: {
    ...
    const headers = ['Date', 'Traffic Type', 'Plays', 'Engagement']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
