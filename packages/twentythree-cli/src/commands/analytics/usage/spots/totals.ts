...
/**
 * Analytics usage spots totals command — returns aggregated spot usage analytics.
 *
 * Uses GET /analytics/data/usage/spots/totals.
 * No pagination — the totals endpoint does not support p/size.
 */
export default class AnalyticsUsageSpotsTotals extends AuthenticatedCommand<typeof AnalyticsUsageSpotsTotals> {
  static description = 'Get aggregated spot usage analytics - totals'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Spot', 'Plays', 'Engagement', 'Traffic'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageSpotsTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/spots/totals', {
      params: {
        query: {
    ...
    const headers = ['Spot', 'Plays', 'Engagement', 'Traffic']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
