...
/**
 * Analytics usage spots timeseries command — returns spot usage analytics time series data.
 *
 * Uses GET /analytics/data/usage/spots/timeseries.
 * No pagination — the timeseries endpoint does not support p/size.
 */
export default class AnalyticsUsageSpotsTimeseries extends AuthenticatedCommand<typeof AnalyticsUsageSpotsTimeseries> {
  static description = 'Get spot usage analytics - time series'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Date', 'Spot', 'Plays', 'Engagement'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageSpotsTimeseries)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/spots/timeseries', {
      params: {
        query: {
    ...
    const headers = ['Date', 'Spot', 'Plays', 'Engagement']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
