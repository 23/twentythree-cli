...
/**
 * Analytics usage devices command — returns usage analytics broken down by device type.
 *
 * Uses GET /analytics/data/usage/devices.
 * Supports pagination via p/size query params.
 */
export default class AnalyticsUsageDevices extends AuthenticatedCommand<typeof AnalyticsUsageDevices> {
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageDevices)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/devices', {
      params: {
        query: {
