...
/**
 * Analytics usage domains command — returns usage analytics broken down by domain.
 *
 * Uses GET /analytics/data/usage/domains.
 * Supports pagination via p/size query params.
 */
export default class AnalyticsUsageDomains extends AuthenticatedCommand<typeof AnalyticsUsageDomains> {
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageDomains)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/domains', {
      params: {
        query: {
    ...
    const headers = ['Domain', 'Plays', 'Engagement', 'Traffic', 'Impressions']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
