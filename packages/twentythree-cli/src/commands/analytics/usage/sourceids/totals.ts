...
/**
 * Analytics usage sourceids totals command — returns aggregated usage analytics by source ID.
 *
 * Uses GET /analytics/data/usage/sourceids/totals.
 * No pagination — the totals endpoint does not support p/size.
 */
export default class AnalyticsUsageSourceidsTotals extends AuthenticatedCommand<typeof AnalyticsUsageSourceidsTotals> {
  static description = 'Get aggregated usage analytics by source ID - totals'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Source ID', 'Plays', 'Engagement', 'Traffic'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageSourceidsTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/sourceids/totals', {
      params: {
        query: {
    ...
    const headers = ['Source ID', 'Plays', 'Engagement', 'Traffic']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
