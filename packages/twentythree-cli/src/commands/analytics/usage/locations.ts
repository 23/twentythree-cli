...
/**
 * Analytics usage locations command — returns usage analytics broken down by location.
 *
 * Uses GET /analytics/data/usage/locations.
 * Supports pagination via p/size query params.
 */
export default class AnalyticsUsageLocations extends AuthenticatedCommand<typeof AnalyticsUsageLocations> {
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Location', 'Plays', 'Engagement', 'Traffic', 'Impressions'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageLocations)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/locations', {
      params: {
        query: {
    ...
    const headers = ['Location', 'Plays', 'Engagement', 'Traffic', 'Impressions']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const tableRows = rows.map((row: any) => [
      String(row.location ?? row.country ?? ''),
      String(row.plays ?? ''),
