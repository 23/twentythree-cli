...
/**
 * Analytics usage domains totals command — returns aggregated usage analytics by embed domain.
 *
 * Uses GET /analytics/data/usage/domains/totals.
 * No pagination — the totals endpoint does not support p/size.
 */
export default class AnalyticsUsageDomainsTotals extends AuthenticatedCommand<typeof AnalyticsUsageDomainsTotals> {
  static description = 'Get aggregated usage analytics by embed domain - totals'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Domain', 'Plays', 'Engagement', 'Traffic'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsUsageDomainsTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/usage/domains/totals', {
      params: {
        query: {
    ...
    const headers = ['Domain', 'Plays', 'Engagement', 'Traffic']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
