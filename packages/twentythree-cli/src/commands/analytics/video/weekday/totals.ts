...
/**
 * Analytics video weekday totals command — returns aggregated video analytics by weekday totals.
 *
 * Uses GET /analytics/data/videos/weekday/totals (plural 'videos' per OpenAPI spec).
 * No pagination — the totals endpoint does not support p/size.
 */
export default class AnalyticsVideoWeekdayTotals extends AuthenticatedCommand<typeof AnalyticsVideoWeekdayTotals> {
  static description = 'Get aggregated video analytics by weekday - totals'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Day', 'Plays', 'Engagement', 'Playrate'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsVideoWeekdayTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/videos/weekday/totals', {
      params: {
        query: {
    ...
    const headers = ['Day', 'Plays', 'Engagement', 'Playrate']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
