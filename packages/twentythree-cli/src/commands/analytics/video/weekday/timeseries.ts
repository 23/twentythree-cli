...
/**
 * Analytics video weekday timeseries command — returns video analytics by weekday time series data.
 *
 * Uses GET /analytics/data/videos/weekday/timeseries (plural 'videos' per OpenAPI spec).
 * No pagination — the timeseries endpoint does not support p/size.
 */
export default class AnalyticsVideoWeekdayTimeseries extends AuthenticatedCommand<typeof AnalyticsVideoWeekdayTimeseries> {
  static description = 'Get video analytics by weekday - time series'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Date', 'Day', 'Plays', 'Engagement', 'Playrate'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsVideoWeekdayTimeseries)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/videos/weekday/timeseries', {
      params: {
        query: {
    ...
    const headers = ['Date', 'Day', 'Plays', 'Engagement', 'Playrate']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
