...
/**
 * Analytics video published totals command — returns aggregated video analytics by publish date totals.
 *
 * Uses GET /analytics/data/videos/published/totals (plural 'videos' per OpenAPI spec).
 * No pagination — the totals endpoint does not support p/size.
 */
export default class AnalyticsVideoPublishedTotals extends AuthenticatedCommand<typeof AnalyticsVideoPublishedTotals> {
  static description = 'Get aggregated video analytics by publish date - totals'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Published', 'Plays', 'Engagement'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsVideoPublishedTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/videos/published/totals', {
      params: {
        query: {
    ...
    const headers = ['Published', 'Plays', 'Engagement']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
