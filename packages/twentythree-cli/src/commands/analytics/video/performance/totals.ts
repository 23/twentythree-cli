...
/**
 * Analytics video performance totals command — returns aggregated video playthrough performance totals.
 *
 * Uses GET /analytics/data/videos/performance/totals (plural 'videos' per OpenAPI spec).
 * No pagination — the totals endpoint does not support p/size.
 */
export default class AnalyticsVideoPerformanceTotals extends AuthenticatedCommand<typeof AnalyticsVideoPerformanceTotals> {
  static description = 'Get aggregated video playthrough performance - totals'
  ...
  static agentMetadata = {
    ...
    output_shape: { type: 'table' as const, columns: ['Plays', 'Engagement', 'Playthrough'] },
  ...
  public async run(): Promise<void | object> {
    const { flags } = await this.parse(AnalyticsVideoPerformanceTotals)

    this.printWorkspaceHeader()

    const { data, error } = await this.apiClient.GET('/analytics/data/videos/performance/totals', {
      params: {
        query: {
    ...
    const headers = ['Plays', 'Engagement', 'Playthrough']
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
